...
import { Menu, LogOut, User, Search, Eye, LogIn, UserPlus } from 'lucide-react';
    ...
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-x-4 border-b border-white/[0.07] bg-[#0b0d14]/80 backdrop-blur-xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-1 items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          {/* Guest mode badge */}
          {isGuest && (
            <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/12 border border-amber-500/25 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-400">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 sm:gap-2.5 rounded-lg bg-white/[0.03] border border-white/[0.07] px-2.5 sm:px-3 py-1.5 text-xs text-slate-400 hover:border-white/[0.16] hover:bg-white/[0.06] hover:text-slate-200 transition-all cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"
            aria-label="Quick search (Press Ctrl+K or Cmd+K)"
            title="Search workspace (⌘K)"
          >
            <Search className="h-3.5 w-3.5 text-slate-400" />
            <span className="text-slate-400 hidden sm:inline">Quick search…</span>
          ...
          {isGuest ? (
            /* ── Guest top-right: Sign In + Create Account CTAs ── */
            <div className="flex items-center gap-2">
              <button
                onClick={handleExitGuest}
                className="hidden sm:inline-flex items-center gap-1.5 rounded-lg border border-white/[0.1] bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-white/[0.07] hover:text-white transition-all"
              >
              ...
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 px-3 py-1.5 text-xs font-semibold text-white shadow-md shadow-violet-600/20 transition-all"
              >
